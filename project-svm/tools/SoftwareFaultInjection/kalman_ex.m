...
QuailAccel_noise_mag = 3.50 + loop*2; %process noise: the variability in how fast the Quail is speeding up (stdv of acceleration: meters/sec^2)
NinjaVision_noise_mag = 0.30;  %measurement noise: How mask-blinded is the Ninja (stdv of location, in meters)
...
for t = 1:length(Q_loc)
    ...
    if t == bit_flip_time
        Q_loc_meas_fault = bit_flip_random(Q_loc_meas(t), 256.00, 16, 1, 'Q_loc_meas(t)');
        ...
        %end
        Q_estimate_no_fault = Q_estimate + K * (Q_loc_meas(t) - C * Q_estimate);
        Q_estimate = Q_estimate + K * (Q_loc_meas_fault - C * Q_estimate);
        %fprintf('Q_estimate(1): %f => %f\n', Q_estimate_no_fault(1), Q_estimate(1));
        %fprintf('Q_estimate(2): %f => %f\n', Q_estimate_no_fault(2), Q_estimate(2));
        measurement_diff = abs(Q_loc_meas_fault - Q_loc_meas(t))/Q_loc_meas(t);
